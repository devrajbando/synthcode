...
const TreeIndicator = forwardRef(({ className, ...props }, ref) => {
  ...
  return (
    (<div
      ...
      ref={ref}
      className={cn(
        "absolute left-1.5 h-full w-px rounded-md bg-zinc-100 py-3 duration-300 ease-in-out hover:bg-slate-300 rtl:right-1.5 dark:bg-zinc-800",
        className
      )}
      {...props} />)
  );
    ...
    (<AccordionPrimitive.Item {...props} value={value} className="relative h-full overflow-hidden">
      <AccordionPrimitive.Trigger
        className={cn(`flex items-center gap-1 rounded-md text-sm`, className, {
          "bg-zinc-100 rounded-md dark:bg-zinc-800": isSelect && isSelectable,
  ...
  return (
    (<button
      ...
      className={cn(
        ...
        {
          "bg-zinc-100 dark:bg-zinc-800": isSelected && isSelectable,
        },
